...
import { NoteProps } from '../pages/Notepad'

interface Props extends NoteProps {
  removeNote: (id: string) => void;
}

export const Note = ({ id, text, date, removeNote }: Props) => {
  const onCopy = () => {
    copy(text)
  ...
  return (
    <>
      <div className="note flex my-4  text-sm  bg-slate-300 p-4  rounded flex-col w-80 md:w-96  ">
        <span className="flex flex-wrap text-left mb-16 break-all whitespace-pre-line">
          {text}
        </span>
